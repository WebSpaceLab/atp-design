  ...
  <XModal :show="isOpenAgreementModel">
    <div class="w-full relative  p-4">
      <div class="absolute top-2 right-4 flex items-center space-x-2">
        <x-select v-model="lang" label="Language" :options="optionsSelected" />
      </div>
      ...
      <div class="w-full mt-4 flex justify-between">
        <x-btn variant="solid" color="red" label="Reject" @click="agreement(false)" />

        <x-btn variant="solid" color="primary" label="Agree" @click="agreement(true)" />
      </div>
    </div>
  </XModal>
</div>
