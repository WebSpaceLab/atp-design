...
<template>
  <li
    class="w-full ease-in list-none text-primary-600"
    :class="sidebar.isRail ? '' : 'pl-5'"
  >
    <XLink
    ...
    >
      ...
      <span
        v-if="!sidebar.isRail"
        class="w-full ml-3 transition-all duration-500"
      >
        {{ link.label }}
      </span>
    </XLink>
    ...
    <div
      v-else
      class="w-full h-full flex flex-between items-center"
    >
      <XLink
        :to="{ path: link.to }"
        :class="[
      ...
      >
        <div
          v-if="!sidebar.isRail"
          class="flex items-start transition-all duration-500"
        >
      ...
      >
        <Icon
          v-if="sidebar.isRail && !open"
          class="text-2xl transition-all duration-500 hover:text-hover-200"
          :name="link.icon"
        />
    ...
    >
      <ul
        v-if="link?.children?.length && open"
        :class="[sidebar.isRail ? 'w-10 p-0' : 'w-[90%] pl-5']"
        class="list-none"
      >
