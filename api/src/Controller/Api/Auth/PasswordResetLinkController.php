...
class PasswordResetLinkController extends AbstractAPIController
{
    ...
    #[Route('/api/forgot-password', name: 'forgot.password')]
    public function forgetPassword(Request $request, ValidatorInterface $validator, UserRepository $userRepository): JsonResponse
    {
        ...
        return $this->api(['message' => 'Mail z linkiem do resetu hasła został wysłany. Proszę sprwdź konto mailowe.']);
    }
    ...
    {
        $url = $this->generateUrl('verify.email:forget.password', ['token' => $resetPasswordToken], UrlGeneratorInterface::ABSOLUTE_URL);
    ...
    #[Route('/api/forgot-password/verify-email/', name: 'verify.email:forget.password')]
    public function verifyUserEmail(Request $request, UserRepository $userRepository): RedirectResponse
    {
        ...
        return $this->redirectToClientRoute('/password-reset/' . $resetPasswordToken->getToken());
    }
